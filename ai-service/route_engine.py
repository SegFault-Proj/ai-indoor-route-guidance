...
def calculate_shortest_route(
    ...
    *,
    ...
    queue_index = 0
    queue: list[tuple[float, int, str, str | None]] = [(0.0, queue_index, start_id, None)]
    best_destination_state: tuple[str, str | None] | None = None

    while queue:
        current_cost, _queue_index, current_id, current_edge_id = heappop(queue)
        ...
        for neighbor in graph[current_id]:
            ...
            if next_cost < costs.get(next_state, float("inf")):
                ...
                queue_index += 1
                heappush(queue, (next_cost, queue_index, next_id, next_edge_id))
