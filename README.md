...
팀 프론트와 연결할 때는 백엔드 실행 환경에 `ALLOWED_ORIGINS`를 추가하세요.
예를 들어 React 프론트가 `http://localhost:3000`에서 뜨면 다음처럼 설정합니다.

```powershell
$env:ALLOWED_ORIGINS="http://localhost:3000,http://localhost:5173"
```

프론트 쪽에서는 API 주소를 `VITE_AI_API_URL`로 맞추면 됩니다.
...
프론트가 별도 레포에 있다면 백엔드 `ALLOWED_ORIGINS`와 프론트 `VITE_AI_API_URL`
두 값만 먼저 맞추면 됩니다.
...
5. `ALLOWED_ORIGINS`를 배포 도메인에 맞게 변경
